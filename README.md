...
> **Video Walkthrough:** [Watch demo on Loom](https://www.loom.com/share/b362dc701ceb4e2bb32063a7748c78c1)
>
...
|----------|--------------|-----|----------------|
| **PDF parsing** | PyMuPDF (fitz) | Lightweight (single pip install, ~200MB Docker image), fast, handles digital presentation PDFs cleanly. Unstructured.io adds heavy system deps (poppler, tesseract, libmagic) bloating Docker to 2-3GB+ — overkill for digital PDFs with clean text layers | Switch to Unstructured.io if ingesting scanned documents or complex multi-column layouts that need OCR and ML-based layout detection |
...
| **Context window management** | Token budget (12K) per tool + 4K max output | Chunks are included in rank order until the budget is reached — most relevant chunks always make it in. Prevents context overflow while keeping prompt lean for faster/cheaper generation | Add proper tokenizer (tiktoken) for exact counts instead of word-based estimation |
...
- **Embedding rate limits**: Google's free tier allows 100 requests/minute. Ingestion takes ~150s for 3 documents due to rate-limit pacing. The system handles this automatically with rate limiting and retry.
